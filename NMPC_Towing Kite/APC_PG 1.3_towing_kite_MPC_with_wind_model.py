...
def plotOpenLoop(opt_x_k, ambient_wind):
    ...
    tether_force = [-stage_cost_fcn(X_k.T[i,:], U_k.T[i], E_k.T[i], ambient_wind) for i in range(SIZE)]
...
def plotClosedLoop(res_x_mpc, res_u_mpc, res_e_mpc, wind_speeds):
    ...
    tether_force = [-stage_cost_fcn(res_x_mpc.T[i,:],res_u_mpc.T[i],res_e_mpc.T[i],wind_speeds.T[i]) for i in range(SIZE)]
...
v0 = SX.sym('v0', 1)            #variable wind speed
...
ambient_wind = 10               #constant ambient wind speed
wind = Wind(ambient_wind, dt)   #wind model
...
system = Function("sys", [x,u,v0], [xdot])

# Create the ODE and an IDAS integrator for system simulation
ode = {'x': x, 'ode': xdot, 'p': vertcat(u,v0)}
opts = {'tf': dt}
...
stage_cost_fcn = Function('stage_cost_fcn',[x,u,epsilon,v0],[stage_cost])
...
for i in range(N):
    ...
    J += stage_cost_fcn(opt_x['x',i,0], opt_x['u',i], opt_x['epsilon',i], v0)
    ...
    for k in range(1,K+1):
        gk = -dt*system(opt_x['x',i,k], opt_x['u',i], v0)
        for j in range(K+1):
...
prob = {'f':J, 'x':vertcat(opt_x), 'g':g, 'p':vertcat(x_init,v0)}
...
mpc_res = mpc_solver(p=vertcat(x_0,ambient_wind), x0=opt_x_k, lbg=lb_g, ubg=ub_g, lbx = lb_opt_x, ubx = ub_opt_x)
opt_x_k = opt_x(mpc_res['x'])

# Plot the open-loop trajectories
plotOpenLoop(opt_x_k, ambient_wind)
...
for i in range(N_sim):
    ...
    wind_speed = wind.make_step()
    mpc_res = mpc_solver(p=vertcat(x_0,wind_speed), x0=opt_x_k, lbg=lb_g, ubg=ub_g, lbx = lb_opt_x, ubx = ub_opt_x)
    ...
    res_integrator = ode_solver(x0=x_0, p=vertcat(u_k,wind_speed))
...
plotClosedLoop(res_x_mpc, res_u_mpc, res_e_mpc, wind_speeds)
