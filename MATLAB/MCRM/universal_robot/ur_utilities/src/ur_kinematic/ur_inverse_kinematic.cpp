...
URInverseKinematic::URInverseKinematic() : RobotInverseKinematic(std::make_shared<URKinematic>()) { parseLinkLengths(); }

URInverseKinematic::URInverseKinematic(URKinematic::Ptr ur_kinematic) : RobotInverseKinematic(ur_kinematic) { parseLinkLengths(); }

void URInverseKinematic::setDesiredLastJoint(double value) { _q6_desired = value; }

void URInverseKinematic::calculateIKSolution()
{
    if (!_initialized)
    {
    ...
    defineTransformation(_task_position, _task_orientation_rpy, T);
    _n_solutions = inverse(T, ik_solution);

    if (_n_solutions > 0)
    {
        // interval [-pi,pi]:
        for (int i = 0; i < (_n_solutions * 6); ++i)
        {
            if (ik_solution[i] > M_PI) ik_solution[i] -= (2 * M_PI);
        }

        // store solution in matrix form
        _solutions.resize(_n_solutions, _n_dynamic_joints);

        for (int i = 0; i < _n_solutions; ++i)
        {
            for (int j = 0; j < 6; ++j)
            {
                _solutions(i, j) = (ik_solution[i * 6 + j]);
            }
        }
    }
}
