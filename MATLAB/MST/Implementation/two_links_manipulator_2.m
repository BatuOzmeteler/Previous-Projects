...
% Resulting in the Vertical Motion of the End-Effector
%-------------------------------------------------------------------------%

function two_links_manipulator_2
...
for i = 1:xpoints
    Ff = [0; -(cos(p3(i))*sin(p4(i)) - cos(p4(i))*sin(p3(i))); -sin(p4(i)); sin(p3(i))];     
...
for i = 1:(xpoints-1)
    ...
    Ff_combined = [Ff zeros(4,1)];
    
    xstar(:,i+1) = xstar(:,i) + Ff_combined*u(:,i)*(x(i+1)-x(i));
