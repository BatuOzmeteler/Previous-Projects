...
public class pastAppointmentsServlet extends HttpServlet {
    ...
    protected void pageData(HttpServletRequest request, HttpServletResponse response, int filter_set,
        ...
        if (HMSUtils.checkCookies(request, "Doctor")) {
            ...
            ResultSet d_pastAppos = dbManager.get_Appos(username, 3, filter_set, filter);
            request.setAttribute("Doctor Past Appos", d_pastAppos);
            request.getRequestDispatcher("/pastAppointments.jsp").forward(request, response);
        } else {
